...
import com.animania.addons.extra.client.model.rabbits.ModelRabbitLop;
...
public class RenderRabbitLop<T extends EntityAnimaniaRabbit> extends MobRenderer<T, ModelRabbitLop<T>>
{
	public static final ModelLayerLocation LAYER = new ModelLayerLocation(
			ResourceLocation.fromNamespaceAndPath(Animania.MODID, "rabbit_lop"), "main");
	private static final ResourceLocation[] TEXTURES = {
		texture("rabbit_lop_black"), texture("rabbit_lop_brown"), texture("rabbit_lop_golden"), texture("rabbit_lop_olive"),
		texture("rabbit_lop_patch_black"), texture("rabbit_lop_patch_brown"), texture("rabbit_lop_patch_grey")
	};
	private static final ResourceLocation KILLER = texture("rabbit_killer");

	public RenderRabbitLop(EntityRendererProvider.Context context)
	{
		super(context, new ModelRabbitLop<>(context.bakeLayer(LAYER)), .25F);
	...
	public ResourceLocation getTextureLocation(T rabbit)
	{
		if (isKiller(rabbit))
			return KILLER;
		return TEXTURES[Math.floorMod(rabbit.getColorNumber(), TEXTURES.length)];
	}
	...
	protected void scale(T rabbit, PoseStack poseStack, float partialTick)
	{
		...
		if (rabbit instanceof EntityRabbitKitBase kit)
			scale = .23F + kit.getEntityAge() / kit.getSizeDividend();
		else if (isKiller(rabbit))
			scale = .7F;
		else
			scale = rabbit instanceof EntityRabbitBuckBase ? .47F : .51F;
