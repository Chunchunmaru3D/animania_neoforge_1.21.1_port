...
import com.animania.addons.extra.client.model.rabbits.ModelRabbitDutch;
...
public class RenderRabbitDutch<T extends EntityAnimaniaRabbit> extends MobRenderer<T, ModelRabbitDutch<T>>
{
	public static final ModelLayerLocation LAYER = new ModelLayerLocation(
			ResourceLocation.fromNamespaceAndPath(Animania.MODID, "rabbit_dutch"), "main");
	private static final ResourceLocation TEXTURE = texture("rabbit_dutch");
	private static final ResourceLocation KILLER = texture("rabbit_killer");

	public RenderRabbitDutch(EntityRendererProvider.Context context)
	{
		super(context, new ModelRabbitDutch<>(context.bakeLayer(LAYER)), .25F);
	...
	protected void scale(T rabbit, PoseStack poseStack, float partialTick)
	{
		...
		if (rabbit instanceof EntityRabbitKitBase kit)
			...
			scale = rabbit instanceof EntityRabbitBuckBase ? .52F : .53F;
