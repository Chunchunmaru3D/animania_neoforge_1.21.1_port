...
import com.animania.addons.extra.client.model.rabbits.ModelRabbitCottontail;
...
public class RenderRabbitCottontail<T extends EntityAnimaniaRabbit> extends MobRenderer<T, ModelRabbitCottontail<T>>
{
	public static final ModelLayerLocation LAYER = new ModelLayerLocation(
			ResourceLocation.fromNamespaceAndPath(Animania.MODID, "rabbit_cottontail"), "main");
	private static final ResourceLocation TEXTURE = texture("rabbit_cottontail");
	private static final ResourceLocation KILLER = texture("rabbit_killer");

	public RenderRabbitCottontail(EntityRendererProvider.Context context)
	{
		super(context, new ModelRabbitCottontail<>(context.bakeLayer(LAYER)), .25F);
	...
	protected void scale(T rabbit, PoseStack poseStack, float partialTick)
	{
		...
		if (rabbit instanceof EntityRabbitKitBase kit)
			scale = .26F + kit.getEntityAge() / kit.getSizeDividend();
		else if (isKiller(rabbit))
			scale = .7F;
		else
			scale = rabbit instanceof EntityRabbitBuckBase ? .52F : .56F;
