...
import com.animania.addons.extra.client.model.rabbits.ModelRabbitNewZealand;
...
public class RenderRabbitNewZealand<T extends EntityAnimaniaRabbit> extends MobRenderer<T, ModelRabbitNewZealand<T>>
{
	public static final ModelLayerLocation LAYER = new ModelLayerLocation(
			ResourceLocation.fromNamespaceAndPath(Animania.MODID, "rabbit_new_zealand"), "main");
	private static final ResourceLocation TEXTURE = texture("rabbit_new_zealand");
	private static final ResourceLocation KILLER = texture("rabbit_killer");

	public RenderRabbitNewZealand(EntityRendererProvider.Context context)
	{
		super(context, new ModelRabbitNewZealand<>(context.bakeLayer(LAYER)), .25F);
	...
	protected void scale(T rabbit, PoseStack poseStack, float partialTick)
	{
		...
		if (rabbit instanceof EntityRabbitKitBase kit)
			...
			scale = rabbit instanceof EntityRabbitBuckBase ? .57F : .58F;
