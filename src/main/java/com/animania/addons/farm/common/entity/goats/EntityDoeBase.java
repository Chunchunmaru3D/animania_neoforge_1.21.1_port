...
public class EntityDoeBase extends EntityAnimaniaGoat implements IMateable, IImpregnable
{
	protected static final EntityDataAccessor<Optional<UUID>> MATE_UNIQUE_ID = SynchedEntityData.defineId(EntityDoeBase.class, EntityDataSerializers.OPTIONAL_UUID);
	...
	protected static final EntityDataAccessor<Integer> GESTATION_TIMER = SynchedEntityData.defineId(EntityDoeBase.class, EntityDataSerializers.INT);

	public int dryTimerDoe;

	public EntityDoeBase(EntityType<? extends EntityDoeBase> entityType, Level level)
	{
		...
		if (this.getFed() && this.getWatered() && !stack.isEmpty() && this.getHasKids())
		{
			if (stack.is(Items.BUCKET))
			{
				if (!this.level().isClientSide)
				{
					...
					stack.shrink(1);
					AnimaniaHelper.addItem(player, new ItemStack(com.animania.addons.farm.FarmItems.GOAT_MILK_BUCKET.get()));
	...
	@Override
	public EntityDoeBase getBreedOffspring(net.minecraft.server.level.ServerLevel level, net.minecraft.world.entity.AgeableMob otherParent)
	{
		return null;
	}
}
